...
Future versions of cf-networking will migrate the source of truth for security groups to policy-server and elimintate the need to poll capi (after which this document can be deleted).
...
First Query (page=1, page_size=5000):
...
Q: Why is this complex pagination necessary?
...
Q: Why do we have to decrement page size for each page?
...
Q: Why do we have to increment the index of the ASG we are inspecting?
...
Q: What happens when there are +5000 pages?
...
Q: How long will this algorithm be in place?
