...
var migration_v0082 = map[string][]string{
	"mysql": {
		`CREATE TABLE  IF NOT EXISTS staging_spaces (
			id bigint NOT NULL AUTO_INCREMENT,
			security_group_guid varchar(36) NOT NULL,
			space_guid varchar(36) NOT NULL,
			PRIMARY KEY (id),
			UNIQUE KEY staging_group_space (security_group_guid, space_guid),
			KEY staging_security_group_guid_fkey (security_group_guid),
			CONSTRAINT staging_security_group_guid_fkey
