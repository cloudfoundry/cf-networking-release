...
var _ = Describe("GetConnectionPool", func() {
	...
	GetConnectionPoolAssertions := func() {
	...
	Context("in Postgres", func() {
		BeforeEach(func() {
			dbConnectionInfo := testsupport.GetPostgresDBConnectionInfo()
			testDatabase = dbConnectionInfo.CreateDatabase(dbName)
		})

		GetConnectionPoolAssertions()
	})

	Context("in MySQL", func() {
		BeforeEach(func() {
			dbConnectionInfo := testsupport.GetMySQLDBConnectionInfo()
			testDatabase = dbConnectionInfo.CreateDatabase(dbName)
		})
